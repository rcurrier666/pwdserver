...
describe ( 'User functions', ( ) => {
  ...
  it ( '/users/:uid - success', ( done ) => {
      request ( 'http://localhost:3000/users/1001', ( error, response, body ) => {
  ...
  it ( '/users/:uid - failure', ( done ) => {
      request ( 'http://localhost:3000/users/55', ( error, response, body ) => {
  ...
  it ( '/users/query?name=testuser - success', ( done ) => {
      request ( 'http://localhost:3000/users/query?name=testuser', ( error, response, body ) => {
  ...
  it ( '/users/query?name=testuser&uid=1002 - success', ( done ) => {
      request ( 'http://localhost:3000/users/query\?name=testuser&uid=1002', ( error, response, body ) => {
          expect ( body ).to.equal ( '[{"name":"testuser","uid":1002,"gid":1002,"comment":"Test User","home":"/home/testuser","shell":"/bin/ksh"}]' );
          expect ( response.statusCode ).to.equal ( 200 );
          done ( );
      });
  ...
  it ( '/users/query?name=testuser&uid=99 - failure', ( done ) => {
      request ( 'http://localhost:3000/users/query?name=testuser&uid=99', ( error, response, body ) => {
          expect ( body ).to.equal ( '404 - Not found' );
          expect ( response.statusCode ).to.equal ( 404 );
          done ( );
      });
  ...
  it ( '/users/:uid/groups - success', ( done ) => {
      request ( 'http://localhost:3000/users/1001/groups', ( error, response, body ) => {
  ...
  it ( '/users/:uid/groups - failure', ( done ) => {
      request ( 'http://localhost:3000/users/99/groups', ( error, response, body ) => {
