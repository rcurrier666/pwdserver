...
describe('Group functions', () => {
  ...
  it('/groups/:gid - success', (done) => {
      request('http://localhost:3000/groups/1001', function(error, response, body) {
  ...
  it('/groups/:gid - failure', (done) => {
      request('http://localhost:3000/groups/55', function(error, response, body) {
  ...
  it('/groups/query?name=testuser - success', (done) => {
      request('http://localhost:3000/groups/query?name=testuser', function(error, response, body) {
  ...
  it('/groups/query?name=testuser&gid=1002 - success', (done) => {
      request('http://localhost:3000/groups/query?name=testuser&gid=1002', function(error, response, body) {
          expect(body).to.equal('[{"name":"testuser","gid":1002,"members":["testuser"]}]');
          expect(response.statusCode).to.equal(200);
          done();
      });
  ...
  it('/groups/query?name=testuser&gid=99 - failure', (done) => {
      request('http://localhost:3000/groups/query?name=testuser&ugd=99', function(error, response, body) {
          expect(body).to.equal('404 - Not found');
          expect(response.statusCode).to.equal(404);
          done();
      });
